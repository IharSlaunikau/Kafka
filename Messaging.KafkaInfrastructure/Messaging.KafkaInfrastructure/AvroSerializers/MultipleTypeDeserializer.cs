...
/// </remarks>
public class MultipleTypeDeserializer<T>(MultipleTypeConfig typeConfig, ISchemaRegistryClient schemaRegistryClient)
    : IAsyncDeserializer<T>
{
    ...
    private async Task<IReaderWrapper> GetReader(int schemaId)
    {
        ...
        try
        {
            if (!_readers.TryGetValue(schemaId, out reader))
            {
                ...
                var registrySchema = await schemaRegistryClient.GetSchemaAsync(schemaId)
                    .ConfigureAwait(continueOnCapturedContext: false);

                var avroSchema = Avro.Schema.Parse(registrySchema.SchemaString);

                reader = typeConfig.CreateReader(avroSchema);
    ...
    {
        if (_readers.Count > schemaRegistryClient.MaxCachedSchemas)
        {
