...
public class MultipleTypeSerializer<T> : IAsyncSerializer<T>
{
    ...
    public async Task<byte[]> SerializeAsync(T data, SerializationContext context)
    {
        ...
        {
            throw new ArgumentException($"Object being serialized is not an instance of {nameof(ISpecificRecord)}." +
                                        $" This serializer only serializes types generated using the avrogen.exe tool.", nameof(data));
        }
        ...
        if (!_serializers.TryGetValue(fullName, out var serializer))
        {
            throw new ArgumentException($"Unexpected type {fullName}." +
                                        $" All types to be serialized need to be registered in the {nameof(MultipleTypeConfig)}" +
                                        $" that is supplied to this instance of {nameof(MultipleTypeSerializer<T>)}", nameof(data));
        }
